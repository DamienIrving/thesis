...
By fully exploiting the information available from Fourier analysis, the previous chapter overcame many of the limitations of existing wave identification methods. This methodological advance led to a number new insights into the climatological characteristics of the SH zonal waves and their influence on regional climate variability. Identification of the PSA pattern has traditionally (and almost exclusively) been based on EOF analysis, meaning many of the same limitations apply. In particular, previous studies have only very crudely captured phase variations in the PSA pattern (via the PSA-1 and PSA-2 modes, which are 90$^{\circ}$ out of phase), which makes it difficult to interpret its propagation characteristics. Without a more nuanced understanding of even this most basic wave property, it is difficult to characterise the climatological behaviour of the pattern and its influence on regional climate variability.
